...
public class WeaponController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (isReloading && currentWeapon != null)
        {
            ...
            if (reloadTimer <= 0f)
            {
                ...
                Debug.Log($"Przeładowano {currentWeapon.weaponName}! (Magazynek powraca do {currentWeapon.magazineSize})");
            }
        }
    }
    ...
    private void StartReload()
    {
        ...
        Debug.Log($"reloading ({currentWeapon.reloadTime})");
    }
    ...
    private void ConfigureProjectile(GameObject bullet, WeaponData weaponSettings)
    {
        ...
        {
            actorScript.SetWaitUntilDestroyed(0.1f);
        ...
        Collider2D[] shooterColliders = GetComponentsInParent<Collider2D>();
        Collider2D[] bulletColliders = bullet.GetComponents<Collider2D>();
        foreach (var sCol in shooterColliders)
        {
            foreach (var bCol in bulletColliders)
            {
                if (weaponSettings.isExplosive && bulletColliders.Length > 1 && bCol == bulletColliders[1])
                {
                    continue;
                }
                Physics2D.IgnoreCollision(bCol, sCol);
            }
        }
    }
    ...
    public void TryShoot(Vector2 targetDirection)
    {
        ...
        Debug.Log($"Ammo: {ammoState[currentWeapon.weaponName]}");
